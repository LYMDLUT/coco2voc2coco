...
def save_annotations(filename, objs, filepath,CKimg_dir,CKanno_dir):
...
def showbycv(coco, dataType, img, classes, origin_image_dir,CKimg_dir,CKanno_dir,verbose=False):
    ...
    save_annotations(filename, objs, filepath,CKimg_dir,CKanno_dir)
    if verbose:
        cv2.imshow("img", I)
        cv2.waitKey(0)
...
def get_CK5(origin_anno_dir, origin_image_dir,CKimg_dir,CKanno_dir, verbose=False):
    ...
    for dataType in dataTypes:
        ...
        for imgId in tqdm(imgIds):
            ...
            showbycv(coco, dataType, img, classes, origin_image_dir,CKimg_dir,CKanno_dir,verbose=False)


def main():
    base_dir = './VOCdevkit/VOC2007/coco2017_voc'  # step1 这里是一个新的文件夹，存放转换后的图片和标注
    image_dir = os.path.join(base_dir, 'images')  # 在上述文件夹中生成images，annotations两个子文件夹
    anno_dir = os.path.join(base_dir, 'annotations')
    mkr(image_dir)
    mkr(anno_dir)
    origin_image_dir = './VOCdevkit/VOC2007/JPEGImages'  # step 2原始的coco的图像存放位置
    origin_anno_dir = './'  # step 3 原始的coco的标注存放位置
    verbose = True  # 是否需要看下标记是否正确的开关标记，若是true,就会把标记展示到图片上
    get_CK5(origin_anno_dir, origin_image_dir,image_dir,anno_dir,verbose)
